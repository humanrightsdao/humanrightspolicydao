...
const CreateHelpRequestModal = ({ userProfile, onClose, onSuccess }) => {
  ...
  const handleFileUpload = (e) => {
    ...
    if (oversizedFiles.length > 0) {
      alert(t("file_too_large") || "File must not exceed 20MB");
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const helpRequestData = {
        ...
        country_code: userProfile.country || "EARTH",
      ...
      // Add payment details if filled
      const hasBankDetails = Object.values(formData.bankDetails).some(
        (value) => value.trim() !== "",
      );
      if (hasBankDetails) {
        helpRequestData.bank_details = formData.bankDetails;
      }
      ...
      if (
        ...
        helpRequestData.crypto_wallets = formData.cryptoWallets.filter(
          (w) => w.trim() !== "",
        );
      }

      if (formData.paypalEmail.trim()) {
        helpRequestData.paypal_email = formData.paypalEmail.trim();
      }

      if (uploadedAttachments.length > 0) {
        helpRequestData.attachments = uploadedAttachments;
      }

      console.log("Data to insert:", helpRequestData);

      // FIX: First check authorization
      const {
        data: { user },
        error: authError,
      } = await supabase.auth.getUser();
      if (authError) throw new Error("Authorization error");
      if (!user) throw new Error("User not authorized");

      // Create request in database
      const { data, error: insertError } = await supabase
        .from("help_requests")
        .insert([helpRequestData]) // Use array
        .select()
        .single();
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 text-sm bg-transparent border border-gray-300 dark:border-gray-600 rounded-full focus:ring-1 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-600 dark:focus:border-blue-600 text-gray-900 dark:text-white outline-none transition-colors"
                ...
                {helpTypes.map((type) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={loading}
                      className={`p-1.5 rounded-full border transition-all flex flex-col items-center gap-0.5 relative ${
                        isSelected
            ...
            {/* Payment details - accordion style */}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      <input
                        ...
                        className="px-3 py-2 text-sm bg-transparent border border-gray-300 dark:border-gray-600 rounded-full focus:ring-1 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-600 dark:focus:border-blue-600 text-gray-900 dark:text-white outline-none transition-colors"
                        placeholder="IBAN"
                        disabled={loading}
                      />
                      <input
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {formData.cryptoWallets.map((wallet, index) => (
                        <div key={index} className="flex gap-1.5">
                          <input
                            ...
                            className="flex-1 px-3 py-2 text-sm bg-transparent border border-gray-300 dark:border-gray-600 rounded-full focus:ring-1 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-600 dark:focus:border-blue-600 text-gray-900 dark:text-white outline-none transition-colors"
                            placeholder="0x..."
                            disabled={loading}
                          />
                          <button
                            ...
                            disabled={loading}
                            className="px-3 py-2 bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-full hover:bg-red-200 dark:hover:bg-red-900/30 disabled:opacity-50 transition-colors"
                          >
                      ...
                      <button
                        ...
                        disabled={loading}
                        className="w-full px-3 py-2 border border-dashed border-gray-300 dark:border-gray-600 rounded-full hover:border-gray-400 dark:hover:border-gray-500 text-gray-600 dark:text-gray-400 disabled:opacity-50 transition-colors flex items-center justify-center gap-1.5 text-sm"
                      >
            ...
            <div className="pt-3 border-t dark:border-gray-700">
              ...
              <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {attachments.map((file, index) => (
                      <div key={index} className="relative group">
                        {file.preview ? (
                          <div className="w-full h-24 rounded-full overflow-hidden border border-gray-200 dark:border-gray-700">
                            <img
                              src={file.preview}
                              alt={file.name}
                          ...
                          <div className="w-full h-24 rounded-full border border-gray-200 dark:border-gray-700 flex flex-col items-center justify-center p-2">
                ...
                <button
                  ...
                  className="w-full px-3 py-2.5 border border-dashed border-gray-300 dark:border-gray-600 rounded-full hover:border-gray-400 dark:hover:border-gray-500 text-gray-600 dark:text-gray-400 disabled:opacity-50 transition-colors flex items-center justify-center gap-1.5 text-sm"
                >
        ...
        <div className="p-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
          <div className="flex items-center justify-end gap-3">
            <button
              onClick={onClose}
              disabled={loading}
              className="px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors font-medium disabled:opacity-50"
            >
              {t("cancel") || "Cancel"}
            </button>
            <button
              ...
              disabled={loading || uploadingFiles}
              className="px-6 py-2 bg-gradient-to-r from-blue-950 to-blue-800 dark:from-blue-700 dark:to-blue-800 text-white rounded-full hover:from-blue-700 hover:to-blue-800 dark:hover:from-blue-600 dark:hover:to-blue-700 transition-all duration-200 font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5 min-w-[100px] justify-center text-sm"
            >
